...
| `05_poststratify.R` | Poststratification (standalone, used by `04_fit_mrp.R`) |
| `07_export_estimates.R` | Export estimates to JSON (website) and CSV (download) |
| `08_check_pipeline.R` | 132-check validation pipeline |
| `09_validate_vote_shares.R` | External validation against election results |

## Note on paths

Scripts were originally written to run from within the `german_election_data` repository, where `here::here()` resolved to the GERDA root. The variable `mb_root` was set as `file.path(here::here(), "meinungsbild")`. For standalone use, `mb_root` should be `here::here()`.

Scripts `03_load_covariates.R` and `03b_build_adjacency.R` read election results and shapefiles from the GERDA data directory and require an explicit path to that data.
