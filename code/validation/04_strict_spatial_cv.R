# ==============================================================================
# 04_strict_spatial_cv.R
# Strict spatial CV: held-out county respondents are excluded from model
# estimation entirely. Tests MRP's ability to predict genuinely unseen counties
# (no county-specific RE, no direct respondent data).
#
# Contrast with 03_spec_grid_cv.R, which holds out counties from the evaluation
# metric only (all respondents still enter the model).
# ==============================================================================
...
message("  With county_code: ", sum(!is.na(survey_data$county_code)),
        " (", round(100 * mean(!is.na(survey_data$county_code)), 1), "%)")
...
# 50-cell prediction frame
pred_kreis_50 <- poststrat |>
  group_by(county_code, age_cat, male, educ_label, state_code) |>
  summarise(N = sum(N), .groups = "drop") |>
  left_join(kreis_cov_std |> select(county_code, log_pop_density_z, unemployment_z, share_secondary_z),
            by = "county_code") |>
...
# ---- 2. Reduced specification grid -------------------------------------------

spec_grid <- expand.grid(
  state_slopes = c("none", "age", "both"),
  covariates   = c("none", "unemp_secondary"),
  employment   = c("none", "fe_only"),
...
fit_issue <- function(issue, state_slopes, covariates, employment,
  ...
  # Strict holdout: exclude respondents from held-out counties
  # Keep respondents with NA county_code (state-level only, no county leakage)
  n_before <- nrow(d)
  if (!is.null(holdout_counties)) {
    d <- d |> filter(is.na(county_code) | !(county_code %in% holdout_counties))
  }
  n_after <- nrow(d)
  ...
  if (covariates == "unemp_secondary") {
    cov_cols <- c(cov_cols, "unemployment_z", "share_secondary_z")
    extra_fe <- c("unemployment_z", "share_secondary_z")
  }
  ...
  if (state_slopes == "both")              re_parts <- c(re_parts, "(1|educ_label:state_code)")
  ...
  # Poststratify to ALL 400 counties (held-out counties get allow.new.levels = TRUE)
  ...
  est <- pk |> group_by(county_code) |>
    summarise(estimate = weighted.mean(.pred, N, na.rm = TRUE), .groups = "drop")

  list(estimates = est, n_before = n_before, n_after = n_after)
}
...
eval_spec_fold <- function(spec_row, train_counties, test_counties) {
  results <- future_lapply(vote_issues, function(issue) {
    fit_issue(issue,
              ...
              holdout_counties = test_counties)
  }, future.seed = TRUE)
  ...
  # Report exclusion count from first successful issue
  first_res <- results[[successful[1]]]
  n_excluded <- first_res$n_before - first_res$n_after

  mrp <- map_dfr(successful, ~ mutate(results[[.x]]$estimates, issue_id = .x)) |>
...
message("\nRunning ", nrow(spec_grid), " specs × ", K, " folds = ", n_total,
        " evaluations (strict holdout)\n")

results_list <- list()
pb <- cli_progress_bar("Strict spatial CV", total = n_total, clear = FALSE,
...
write_csv(fold_results, file.path(output_dir, "strict_cv_results.csv"))
...
summary_results <- fold_results |>
  ...
  summarise(
    ...
    mean_excluded  = mean(n_excluded, na.rm = TRUE),
    .groups = "drop"
  ) |>
  arrange(desc(mean_test_r))

write_csv(summary_results, file.path(output_dir, "strict_cv_summary.csv"))

message("\n", strrep("=", 90))
message("STRICT SPATIAL CV: held-out county respondents excluded from estimation")
message(strrep("=", 90))
message(sprintf("\n%-5s  %-14s  %-18s  %-12s  %-10s  %-10s  %-8s  %-10s",
                "Rank", "State slopes", "Covariates", "Employment",
                "Train r", "Test r", "SD", "N excluded"))

for (i in seq_len(nrow(summary_results))) {
  r <- summary_results[i, ]
  message(sprintf("%-5d  %-14s  %-18s  %-12s  %-10.4f  %-10.4f  %-8.4f  %-10.0f",
                  i, r$state_slopes, r$covariates, r$employment,
                  r$mean_train_r, r$mean_test_r, r$sd_test_r, r$mean_excluded))
}

# ---- 8. Compare with specification-selection CV (03) -------------------------

spec_cv_path <- file.path(output_dir, "spec_grid_cv_summary.csv")
if (file.exists(spec_cv_path)) {
  spec_cv <- read_csv(spec_cv_path, show_col_types = FALSE)

  message("\n", strrep("=", 90))
  message("COMPARISON: specification-selection CV vs strict spatial CV")
  message(strrep("=", 90))
  message(sprintf("\n%-14s  %-18s  %-12s  %-12s  %-12s  %-8s",
                  "State slopes", "Covariates", "Employment", "Spec CV r", "Strict CV r", "Gap"))

  for (i in seq_len(nrow(summary_results))) {
    r <- summary_results[i, ]
    match <- spec_cv |>
      filter(state_slopes == r$state_slopes,
             covariates == r$covariates,
             employment == r$employment)
    if (nrow(match) == 1) {
      gap <- match$mean_test_r - r$mean_test_r
      message(sprintf("%-14s  %-18s  %-12s  %-12.4f  %-12.4f  %-8.4f",
                      r$state_slopes, r$covariates, r$employment,
                      match$mean_test_r, r$mean_test_r, gap))
    }
  }
}

message("\nSaved: ", file.path(output_dir, "strict_cv_results.csv"))
message("Saved: ", file.path(output_dir, "strict_cv_summary.csv"))
